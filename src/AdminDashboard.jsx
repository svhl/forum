...
const AdminDashboard = () => {
  ...
  function renderPostBody(body) {
    ...
    return paragraphs.map((para, idx) => (
      <p
        key={idx}
        style={{ margin: 0, marginBottom: "1em", whiteSpace: "pre-wrap" }}
      >
        {para.split("\n").reduce((acc, line, i) => {
  ...
  const handlePost = async () => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/post", {
        ...
        credentials: "include",
        body: JSON.stringify({ topic: activeTab, body: newMessage.trim() }),
      });
  ...
  const handleDelete = async (postId) => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/delete-post", {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
  ...
  return (
    <div className="dashboard-root">
                ...
                {hoveredPostId === post._id && (
                  ...
                  >
                    <div
                      ...
                      onClick={() => handleBanUser(post.username)}
                    >
                      Ban
                    </div>
                    <div
      ...
      {loggedInUser && (
        <aside
          className={`dashboard-rightbar${rightbarVisible ? " visible" : ""}`}
